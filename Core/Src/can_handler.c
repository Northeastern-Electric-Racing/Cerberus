...
void can1_callback(CAN_HandleTypeDef* hcan)
{
	...
	queue_and_set_flag(can_inbound_queue, &new_msg, can_receive_thread, NEW_CAN_MSG_FLAG);
}
...
int8_t queue_can_msg(can_msg_t msg)
{
	...
	return queue_and_set_flag(can_outbound_queue, &msg, can_dispatch_handle, CAN_DISPATCH_FLAG);
}
...
void vCanDispatch(void* pv_params)
{
	...
	for (;;) {
		osThreadFlagsWait(CAN_DISPATCH_FLAG, osFlagsWaitAny, osWaitForever);
		/* Send CAN message */
		while (osMessageQueueGet(can_outbound_queue, &msg_from_queue, NULL, 0U) == osOK) {
