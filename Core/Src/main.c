...
int main(void)
{
	...
	defaultTaskHandle =
		osThreadNew(StartDefaultTask, mpu, &defaultTask_attributes);
	...
	can_handler_args_t *can_handler_args =
		malloc(sizeof(can_handler_args_t));
	assert(can_handler_args);
	can_handler_args->mc = mc;
	can_handler_args->bms = bms;
	can_receive_thread = osThreadNew(vCanReceive, can_handler_args,
					 &can_receive_attributes);
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_3 | GPIO_PIN_8 | GPIO_PIN_9,
			  GPIO_PIN_RESET);
