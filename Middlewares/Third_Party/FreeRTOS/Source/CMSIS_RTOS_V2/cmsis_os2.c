...
#define SVCall_IRQ_NBR \
	(IRQn_Type) -  \
		5 /* SVCall_IRQ_NBR added as SV_Call handler name is not the same for CM0 and for all other CMx */
	...
	if (!IS_IRQ() && (func != NULL)) {
		...
		if (mem == 1) {
			...
			hTask = xTaskCreateStatic(
				(TaskFunction_t)func, name, stack, argument,
...
uint32_t osThreadFlagsSet(osThreadId_t thread_id, uint32_t flags)
{
	...
	if ((hTask == NULL) || ((flags & THREAD_FLAGS_INVALID_BITS) != 0U)) {
		...
		if (IS_IRQ()) {
			...
			(void)xTaskNotifyFromISR(hTask, flags, eSetBits,
						 &yield);
			(void)xTaskNotifyAndQueryFromISR(hTask, 0, eNoAction,
							 &rflags, NULL);
...
uint32_t osThreadFlagsClear(uint32_t flags)
{
	...
	if (IS_IRQ()) {
		...
		if (xTaskNotifyAndQuery(hTask, 0, eNoAction, &cflags) ==
		    pdPASS) {
			rflags = cflags;
			cflags &= ~flags;

			if (xTaskNotify(hTask, cflags,
...
uint32_t osThreadFlagsGet(void)
{
	...
	if (IS_IRQ()) {
		...
		if (xTaskNotifyAndQuery(hTask, 0, eNoAction, &rflags) !=
		    pdPASS) {
			rflags = (uint32_t)osError;
		}
	}
		       ...
		       const osTimerAttr_t *attr)
{
	...
	if (!IS_IRQ() && (func != NULL)) {
		...
		if (callb != NULL) {
			...
			if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
				hTimer = xTimerCreateStatic(
					name, 1, reload, callb, TimerCallback,
					(StaticTimer_t *)attr->cb_mem);
#endif
			} else {
				if (mem == 0) {
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
					hTimer = xTimerCreate(name, 1, reload,
							      callb,
							      TimerCallback);
#endif
				}
...
uint32_t osEventFlagsSet(osEventFlagsId_t ef_id, uint32_t flags)
{
	...
	if ((hEventGroup == NULL) ||
	    ((flags & EVENT_FLAGS_INVALID_BITS) != 0U)) {
		rflags = (uint32_t)osErrorParameter;
	} else if (IS_IRQ()) {
			  ...
			  uint32_t options, uint32_t timeout)
{
	...
	if ((hEventGroup == NULL) ||
		...
		rflags = xEventGroupWaitBits(hEventGroup, (EventBits_t)flags,
					     exit_clr, wait_all,
					     (TickType_t)timeout);
	...
	if (!IS_IRQ()) {
		...
		if ((type & osMutexRobust) != osMutexRobust) {
			...
			if ((hMutex != NULL) && (rmtx != 0U)) {
				hMutex = (SemaphoreHandle_t)((uint32_t)hMutex |
							     1U);
			}
		}
	}
...
osStatus_t osMutexAcquire(osMutexId_t mutex_id, uint32_t timeout)
{
	...
	if (IS_IRQ()) {
		...
		if (rmtx != 0U) {
#if (configUSE_RECURSIVE_MUTEXES == 1)
			if (xSemaphoreTakeRecursive(hMutex, timeout) !=
			    pdPASS) {
				if (timeout != 0U) {
					stat = osErrorTimeout;
				} else {
...
osStatus_t osSemaphoreAcquire(osSemaphoreId_t semaphore_id, uint32_t timeout)
{
	...
	if (hSemaphore == NULL) {
		...
		if (timeout != 0U) {
			...
			if (xSemaphoreTakeFromISR(hSemaphore, &yield) !=
			    pdPASS) {
				stat = osErrorResource;
			} else {
				portYIELD_FROM_ISR(yield);
				     ...
				     const osMessageQueueAttr_t *attr)
{
	...
	if (!IS_IRQ() && (msg_count > 0U) && (msg_size > 0U)) {
		...
		if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
			hQueue = xQueueCreateStatic(msg_count, msg_size,
						    attr->mq_mem, attr->cb_mem);
#endif
		} else {
			if (mem == 0) {
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
			     ...
			     uint8_t msg_prio, uint32_t timeout)
{
	...
	if (IS_IRQ()) {
		if ((hQueue == NULL) || (msg_ptr == NULL) || (timeout != 0U)) {
			...
			if (xQueueSendToBackFromISR(hQueue, msg_ptr, &yield) !=
			    pdTRUE) {
				stat = osErrorResource;
			} else {
				portYIELD_FROM_ISR(yield);
			     ...
			     uint8_t *msg_prio, uint32_t timeout)
{
	...
	if (IS_IRQ()) {
		if ((hQueue == NULL) || (msg_ptr == NULL) || (timeout != 0U)) {
			...
			if (xQueueReceiveFromISR(hQueue, msg_ptr, &yield) !=
			    pdPASS) {
				stat = osErrorResource;
			} else {
				portYIELD_FROM_ISR(yield);
...
#if (configSUPPORT_STATIC_ALLOCATION == 1)
			mp->sem = xSemaphoreCreateCountingStatic(
				block_count, block_count, &mp->mem_sem);
#elif (configSUPPORT_DYNAMIC_ALLOCATION == 1)
			mp->sem = xSemaphoreCreateCounting(block_count,
							   block_count);
#else
			mp->sem == NULL;
#endif
	...
	if (mp_id == NULL) {
		...
		if ((mp->status & MPOOL_STATUS) == MPOOL_STATUS) {
			if (IS_IRQ()) {
				...
				if (xSemaphoreTake(mp->sem,
						   (TickType_t)timeout) ==
				    pdTRUE) {
					if ((mp->status & MPOOL_STATUS) ==
					    MPOOL_STATUS) {
						taskENTER_CRITICAL();
...
osStatus_t osMemoryPoolFree(osMemoryPoolId_t mp_id, void *block)
{
	...
	if ((mp_id == NULL) || (block == NULL)) {
		...
		if ((mp->status & MPOOL_STATUS) != MPOOL_STATUS) {
			...
			if (IS_IRQ()) {
				if (uxSemaphoreGetCountFromISR(mp->sem) ==
				    mp->bl_cnt) {
					stat = osErrorResource;
				} else {
					isrm = taskENTER_CRITICAL_FROM_ISR();
