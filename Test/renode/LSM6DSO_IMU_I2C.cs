...
{
    public class LSM6DSO_IMU_I2C : BasicBytePeripheral, ISPIPeripheral, IProvidesRegisterCollection<ByteRegisterCollection>, ITemperatureSensor, II2CPeripheral
    {
            ...
            firstGroupRegister.DefineMany(this, 6, (register, registerOffset) =>
            {
                ...
                register.WithValueField(0, 8, FieldMode.Read, name: name,
                    valueProviderCallback: _ =>
                    {
                        ...
                        DebugAssert(sample.IsAccelerationSample || sample.IsAngularRateSample, $"Invalid sample: {sample}");
        ...
        {
            public LSM6DSO_FIFO(IMachine machine, LSM6DSO_IMU_I2C owner, string name, uint capacity)
            {
                Capacity = capacity;
            ...
            private readonly LSM6DSO_IMU_I2C owner;
        }
